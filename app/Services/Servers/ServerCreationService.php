...
class ServerCreationService
{
    ...
    public function handle(array $data, DeploymentObject $deployment = null): Server
    {
        ...
        $node = Node::query()->withCount('servers')->find($data['node_id']);
        if ($node->servers_limit && $node->servers_count >= $node->servers_limit) {
            throw new DisplayException('This node has reached its server limit.');
        }
    ...
    private function createModel(array $data): Server
    {
        ...
        $model = $this->repository->create([
            ...
            'database_backup_limit' => Arr::get($data, 'database_backup_limit') ?? 0,
        ]);
