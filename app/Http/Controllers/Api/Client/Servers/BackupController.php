...
use Pterodactyl\Transformers\Api\Client\NodeBackupServerTransformer;
...
class BackupController extends ClientApiController
{
    ...
    /**
     * Returns all the node backups for a given server instance in a paginated
     * result set.
     *
     * @throws AuthorizationException
     */
    public function nodeBackups(Request $request, Server $server): array
    {
        if (!$request->user()->can(Permission::ACTION_BACKUP_READ, $server)) {
            throw new AuthorizationException();
        }
        ...
        $limit = min($request->query('per_page') ?? 20, 50);
        $backups = $server->nodeBackupServers();

        return $this->fractal->collection($backups->paginate($limit))
            ->transformWith($this->getTransformer(NodeBackupServerTransformer::class))
            ->addMeta([
     ...
     * @throws AuthorizationException
     */
    public function downloadNodeBackup(Request $request, Server $server, NodeBackupServer $nodeBackupServer): JsonResponse
    {
        if (!$request->user()->can(Permission::ACTION_BACKUP_DOWNLOAD, $server)) {
            throw new AuthorizationException();
        }

        if ($nodeBackupServer->disk !== Backup::ADAPTER_AWS_S3 && $nodeBackupServer->disk !== Backup::ADAPTER_WINGS) {
            throw new BadRequestHttpException('The backup requested references an unknown disk driver type and cannot be downloaded.');
        }

        $url = $this->nodeBackupService->handleDownload($nodeBackupServer, $request->user());

        Activity::event('server:backup.download')->subject($nodeBackupServer)->property('name', $nodeBackupServer->uuid)->log();
     ...
     * @throws \Throwable
     */
    public function restoreNodeBackup(RestoreBackupRequest $request, Server $server, NodeBackupServer $nodeBackupServer): JsonResponse
    {
        // Cannot restore a backup unless a server is fully installed and not currently
        // processing a different backup restoration request.
        if (!is_null($server->status)) {
        ...
        if (!$nodeBackupServer->is_successful && is_null($nodeBackupServer->completed_at)) {
            throw new BadRequestHttpException('This backup cannot be restored at this time: not completed or failed.');
        }
        ...
        $log = Activity::event('server:backup.restore')
            ->subject($nodeBackupServer)
            ->property(['name' => $nodeBackupServer->uuid, 'truncate' => $truncate]);

        $this->nodeBackupService->handleRestore($nodeBackupServer, $request->user(), false, $truncate);
